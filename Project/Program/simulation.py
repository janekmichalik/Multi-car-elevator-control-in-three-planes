...
class Simulation:
    ...
    def __init__(self):
        ...
        # czekanie 3 windy
        # self.elevators[0].final_path = [[4, 1, 4], [4, 1, 3], [4, 2, 3], [4, 3, 3], [4, 3, 4]]
        # self.elevators[1].final_path = [[4, 1, 2], [4, 1, 3], [4, 2, 3], [4, 3, 3], [4, 4, 3]]
        # self.elevators[2].final_path = [[4, 0, 3], [4, 1, 3], [4, 2, 3], [4, 3, 3], [4, 3, 2]]
        ...
        while pnt <= steps_for_while:
            ...
            if elev1[pnt] == elev2[pnt] == elev3[pnt]:
                ...
                lenn = self.short_for_wait(jinx_elev, not_jinx_elev, cnt, pnt, elevA=elev1,
                                           elevB=elev2, other_elev=elev3)
                if not_jinx_elev.final_path[pnt] == not_jinx_elev.final_path[pnt - 1]:
                    lenn = self.short_for_wait(not_jinx_elev, jinx_elev, cnt, pnt,
                                               find_again=True, elevA=elev1, elevB=elev2, other_elev=elev3)
            elif elev1[pnt] == elev3[pnt]:
                jinx_id = 0 if cnt % 2 == 0 else 2
                not_jinx_id = 2 if jinx_id == 0 else 0
                jinx_elev = self.elevators[jinx_id]
                not_jinx_elev = self.elevators[not_jinx_id]
                lenn = self.short_for_wait(jinx_elev, not_jinx_elev, cnt, pnt,
                                           elevA=elev1, elevB=elev3, other_elev=elev2)
                if not_jinx_elev.final_path[pnt] == not_jinx_elev.final_path[pnt - 1]:
                    lenn = self.short_for_wait(not_jinx_elev, jinx_elev, cnt, pnt,
                                               find_again=True, elevA=elev1, elevB=elev3, other_elev=elev2)
            elif elev2[pnt] == elev3[pnt]:
                jinx_id = 1 if cnt % 2 == 0 else 2
                not_jinx_id = 2 if jinx_id == 1 else 1
                jinx_elev = self.elevators[jinx_id]
                not_jinx_elev = self.elevators[not_jinx_id]
                lenn = self.short_for_wait(jinx_elev, not_jinx_elev, cnt, pnt, elevA=elev2,
                                           elevB=elev3, other_elev=elev1)
                if not_jinx_elev.final_path[pnt] == not_jinx_elev.final_path[pnt - 1]:
                    lenn = self.short_for_wait(not_jinx_elev, jinx_elev, cnt, pnt,
                                               find_again=True, elevA=elev2, elevB=elev3, other_elev=elev1)
            else:
    ...
    @staticmethod
    def short_for_wait(jinx_elev, not_jinx_elev, cnt, pnt, elevA, elevB, other_elev, find_again=False):
